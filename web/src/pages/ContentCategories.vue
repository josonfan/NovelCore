...
import { fetchCategoryList, createCategory, updateCategory, deleteCategory, fetchCategoryDetail } from '../api/categories'
...
const actionButtons = computed(() => {
  ...
  function matchNode(list: any[]): any | null {
    for (const m of list || []) {
      ...
      if (p === path || m?.name === '分类管理') return m
...
async function load() {
  loading.value = true
  try {
    const data = await fetchCategoryList({ page: page.value, limit: limit.value, kw: kw.value })
...
async function onEdit(row: any){
  formMode.value = 'edit'
  try {
    const d: any = await fetchCategoryDetail(row.id)
