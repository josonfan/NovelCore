...
use app\admin\model\SearchConfig;
...
class SearchConfigService
{
    ...
    public static function save(array $data)
    {
        ...
        $m = new SearchConfig();
        if (!empty($exists)) {
            validate(\app\admin\validate\SearchConfig::class)->scene('update')->check($data);
            $id = (int)$exists['id'];
            $m->writeById($id, $data);
            $info = $m->infoById($id);
            return ['mode' => 'updated', 'data' => $info];
        }
        validate(\app\admin\validate\SearchConfig::class)->scene('create')->check($data);
        $m = new SearchConfig($data);
