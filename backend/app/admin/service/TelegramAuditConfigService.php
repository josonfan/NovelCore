...
use app\admin\model\TelegramAuditConfig;
use think\exception\ValidateException;

class TelegramAuditConfigService
{
    public static function getBySiteId($siteId)
    {
        $m = new TelegramAuditConfig();
    ...
    public static function save(array $data)
    {
        ...
        $m = new TelegramAuditConfig();
        if (!empty($exists)) {
            validate(\app\admin\validate\TelegramAuditConfig::class)->scene('update')->check($data);
            $id = (int)$exists['id'];
            $m->writeById($id, $data);
            $info = $m->infoById($id);
            return ['mode' => 'updated', 'data' => $info];
        }
        validate(\app\admin\validate\TelegramAuditConfig::class)->scene('create')->check($data);
        $m = new TelegramAuditConfig($data);
