...
use app\admin\model\CustomerServiceConfig;
...
class CustomerServiceConfigService
{
    ...
    public static function save(array $data)
    {
        ...
        $m = new CustomerServiceConfig();
        if (!empty($exists)) {
            validate(\app\admin\validate\CustomerServiceConfig::class)->scene('update')->check($data);
            $id = (int)$exists['id'];
            $m->writeById($id, $data);
            $info = $m->infoById($id);
            return ['mode' => 'updated', 'data' => $info];
        }
        validate(\app\admin\validate\CustomerServiceConfig::class)->scene('create')->check($data);
        $m = new CustomerServiceConfig($data);
