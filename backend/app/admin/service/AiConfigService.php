...
class AiConfigService
{
    ...
    public static function save(array $data)
    {
        ...
        $m = new AiConfig();
        if (!empty($exists)) {
            validate(\app\admin\validate\AiConfig::class)->scene('update')->check($data);
            $id = (int)$exists['id'];
            $m->writeById($id, $data);
            $info = $m->infoById($id);
            return ['mode' => 'updated', 'data' => $info];
        }
        validate(\app\admin\validate\AiConfig::class)->scene('create')->check($data);
        $m = new AiConfig($data);
