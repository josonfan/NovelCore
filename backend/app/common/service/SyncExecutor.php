...
class SyncExecutor
{
    ...
    protected static function process(array $row): bool
    {
        ...
        if ($row['content_type'] === 'base_config') {
            $payload = json_encode(['type' => (string)$data['config_name'], 'id' => (int)$row['content_id'], 'operation' => (string)$row['operation'], 'data' => $data['config_data']], JSON_UNESCAPED_UNICODE);
        }else{
