...
use app\model\DailyStats as DailyStatsModel;
use app\model\Order;
use app\model\User;
...
class DailyStats extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $record = DailyStatsModel::where('stat_date', $date)->find();
        if ($record) {
            (new DailyStatsModel())->writeById((int)$record->id, $data);
        } else {
            (new DailyStatsModel())->writeById(0, $data);
        }
